...
#define pinGas A0
const int MIN_GAS = 85;
const int MAX_GAS = 370;

#define pinLight A1
const int MIN_LIGHT = 0;
const int MAX_LIGHT = 1010;
...
bool temperatureAlarm(int min, int max, int percentage){
    unsigned int decimal = analogRead(pinTemperature);
...
float convertDecToVolt(unsigned int decimal){
    return (float (decimal) / 1024) * 5;
}
